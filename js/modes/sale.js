  ...
  function bindChargeButton() {
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (!window.posModes?.sale?.openCheckoutModal) {
        window.ui.toast('結帳模組未載入，請重新整理頁面', 'error');
        return;
      }
  ...
  async function confirmCheckout(ctx) {
    ...
    try {
      const order = await window.posApi.createOrder({
        mode: 'sale',
        payment_method: document.getElementById('fieldPayment')?.value || '現金',
        payment_status: '已支付',
        discount: disc,
        note,
        subtotal,
      ...
      if (msg.includes('amount_received') || msg.includes('change_amount')) {
        window.ui.toast('請在 Supabase 執行 database_migration_patch.sql 新增結帳欄位', 'error');
      } else {
        window.ui.toast(`結帳失敗: ${msg}`, 'error');
      }
    } finally {
